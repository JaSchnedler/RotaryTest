...
void OSCMessage::send(UDP &udp, IPAddress remoteIP, unsigned int outPort)
{
    ...
    //prinOutputDatas();
    int addresszeroCount = 0;
    int addressBitCount = 4*round((strlen(outputAddress)+3)/4);
    if(outputAddress == "/Enlight/setDimLevel"){
      addresszeroCount = addressBitCount-16;//-strlen(outputAddress)+2;
      Serial.print("Setting special padding: ");
      Serial.println(addresszeroCount);
    }
    else {
      addresszeroCount = addressBitCount-strlen(outputAddress);
      Serial.print("Setting normal padding: ");
      Serial.println(addresszeroCount);
    }
